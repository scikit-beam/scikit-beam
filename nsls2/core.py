...
def bin_edges(range_min=None, range_max=None, nbins=None, step=None):
    ...
    If `step` is specified then bin width is approximately `step`.  It
    is not exact due to the nature of floats).  There is also a chance
    that if `range_max` is also specified that the last bin may be dropped
    due to rounding errors.  It is far better to specify the other three
    values.

    The arrays generated by `np.cumsum(np.ones(nbins) * step)` and
