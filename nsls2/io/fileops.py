...
GCI: 2/18/14 -- 1) Converted documentation to docstring format and changed file name
    to fileops.py.
...
def load_netCDF(file_name, data_type = None):
    #TODO: Write this as a class so that you can either retreive just the volume or just the dictionary. This should make iterable volume loading more straight forward and easy.
...
def load_reconData(file_name, 
    ...
    """
    #TODO INCORPORATE os.path.splitext(path) and trim code-- will split path so 
    #that file extensions are identified but split, count, search code is 
    #unnecessary.
    ...
    #TODO incorporate if not in "extension list" the raise ValueException("blah 
    #blah blah")
